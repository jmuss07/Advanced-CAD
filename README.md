...
### Reflection
* This assignment used a lot of familiar Onshape features
* In order to bend the ends of the skateboard, you can use the 'Move Face' tool which lets you select a closed off shape/face of a part
* If you choose the 'rotate' setting, you can change the angle to change the bend in the part
* To connect the parts of the skateboard, you can use 'fastened' mates for everything
...
### Reflection 
* Other than the typical sketch and extrude tools, this project also makes use of the 'Shell' and 'Linear Pattern' tools
* It also served as my introduction to variables in Onshape
* Created variables must start with a hashtag (#)
* For this project, you need three variables
    * the height of the brick (#height)
    * the number of rows on the brick (#rows)
    * the number of columns on the brick (#cols)
* The variables created were used to create the bumps on the top of the bricks, as well as the connector pieces on the bottoms of the legos
* The connectors on the bottom of the brick (inside the shell) must have one less connector row and column
    * This was accomplished using a linear pattern with the number of horizontal repeats listed as 
    '#rows-1' and the number of vertical repeats listed as '#cols-1'
...
### Reflection
* This uses something called 'Configurations', which allow you to make multiple versions of the same partin one part studio that you can switch between quickly and easily
* To make a configuration, open the Configurations Tab in the Parts Studio and click the different values that you want to control with configurations, then change the value in the configurations table
...
### Reflection 
* This project makes use of the typical sketch and exrtrude tools, as well as multiple construction lines and patterns
* The multi-tool can be confined to around two sketches
    * one sketch as the base shape for the tool and the inch and centimeter holes
    * the other sketch serves as the pattern for the holes on the protractor part
* To create the curve of the protractor, use the '3-point arc' tool
* A version of the linear pattern tool is available within a sketch, and is what I used to pattern and create the inch and centimeter holes
* On a new layer, make three construction lines starting from the corner of the multitool, and dimension it to be a certain number of degrees from the bottom edge (I used 30°, 45°, and 60°)
* To create the wrench, you can add both the multitool and the desired size of hex-nut to an assembly, then click on the parts and select the 'Edit In Context' tool, and use the 'Use Geometry' tol to create a copy of the dimensions and location of the hex-nut onto your multi-tool so you can extrude the cut
* To end, convert your Part Studio to a Drawing, add any text you want to have etched/engraved onto your multi-tool, export the file, and laser cut it
    * as a note, any lines in your drawing that are red will be cut, and blue lines will be etched/engraved, so make sure any text you add is blue while everything else remains red
    * I would also recommend placing your multi-tool as close to the corner of the Drawing document as you can, and then adding lines going from the edges so it also cuts out a clean edge from the acrylic you're using so that the leftover acrylic can be used for more projects
        * when doing this, make sure that the lines you add are red so that the laser cutter knows to cut through the whole sheet as opposed to just making the surface cuts used for etching and engraving
